...
{{-- ── Filter bar ──────────────────────────────────────────────────────── --}}
@php
    $hasFilters = $activeFilters['city_id'] || $activeFilters['country_id'] || $activeFilters['group_id'];
@endphp
<form method="GET" action="{{ route('admin.super.dashboard') }}" id="dashboard-filter-form"
      class="grid grid-cols-3 gap-4 mb-6">

    {{-- Pays --}}
    <div class="bg-white border border-gray-100 rounded-2xl px-4 py-3 shadow-sm flex items-center gap-3 {{ $activeFilters['country_id'] ? 'border-blue-200 ring-1 ring-blue-100' : '' }}">
        <div class="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 {{ $activeFilters['country_id'] ? 'bg-blue-50' : 'bg-gray-50' }}">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="{{ $activeFilters['country_id'] ? '#3B82F6' : '#9CA3AF' }}" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
        </div>
        <div class="flex-1 min-w-0">
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Pays</p>
            <select name="country_id" onchange="this.form.submit()"
                    class="w-full text-sm font-medium {{ $activeFilters['country_id'] ? 'text-blue-600' : 'text-gray-700' }} bg-transparent focus:outline-none cursor-pointer">
    ...
    <div class="bg-white border border-gray-100 rounded-2xl px-4 py-3 shadow-sm flex items-center gap-3 {{ $activeFilters['city_id'] ? 'border-teal-200 ring-1 ring-teal-100' : '' }}">
        ...
        <div class="flex-1 min-w-0">
            ...
            <select name="city_id" onchange="this.form.submit()"
                    class="w-full text-sm font-medium {{ $activeFilters['city_id'] ? 'text-teal-600' : 'text-gray-700' }} bg-transparent focus:outline-none cursor-pointer">
                <option value="">Toutes les régions</option>
                @foreach($cities as $city)
                <option value="{{ $city->id }}" {{ $activeFilters['city_id'] == $city->id ? 'selected' : '' }}>{{ $city->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    {{-- Groupe --}}
    <div class="bg-white border border-gray-100 rounded-2xl px-4 py-3 shadow-sm flex items-center gap-3 {{ $activeFilters['group_id'] ? 'border-violet-200 ring-1 ring-violet-100' : '' }}">
        <div class="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 {{ $activeFilters['group_id'] ? 'bg-violet-50' : 'bg-gray-50' }}">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="{{ $activeFilters['group_id'] ? '#7C3AED' : '#9CA3AF' }}" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>
        </div>
        <div class="flex-1 min-w-0">
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Groupe</p>
            <select name="group_id" onchange="this.form.submit()"
                    class="w-full text-sm font-medium {{ $activeFilters['group_id'] ? 'text-violet-600' : 'text-gray-700' }} bg-transparent focus:outline-none cursor-pointer">
...
<div class="flex items-center gap-2 -mt-3 mb-5">
    ...
    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-teal-100 text-teal-700">
    ...
    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-violet-100 text-violet-700">
    ...
    <a href="{{ route('admin.super.dashboard') }}"
       class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold text-red-500 bg-red-50 hover:bg-red-100 transition ml-1">
